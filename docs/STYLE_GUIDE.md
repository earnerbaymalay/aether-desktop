...
| `--surface` | `rgba(22, 27, 34, 0.7)` | Header bar, sidebars, panel backgrounds (glassmorphism) |
| `--card` | `rgba(28, 35, 51, 0.5)` | Cards, modals, elevated surfaces (glassmorphism) |
| `--border` | `rgba(48, 54, 61, 0.6)` | All borders, dividers, and separators |
...
| Token | Value | Role |
| :--- | :--- | :--- |
| `--neon-glow` | `0 0 15px rgba(88, 166, 255, 0.3)` | Active element glow effect (`box-shadow`) |
| `--transition-fluid` | `all 0.4s cubic-bezier(0.4, 0, 0.2, 1)` | Standard interaction transition |

> **Note:** `--surface`, `--card`, and `--border` use `rgba()` with alpha transparency to support `backdrop-filter: blur(12px)` glassmorphism throughout the UI. The underlying RGB values match the GitHub Dark High-Contrast theme.
