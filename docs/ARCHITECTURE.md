...
<div align="center">
  <img src="../marketing/advertising_kit/images/ecosystem-graphic.svg" alt="Aether Ecosystem" width="60%" />
</div>
...
| `get_mcp_status` | `() → McpStatus[]` | Returns status of all configured MCP servers |
| `check_api_status` | `() → bool` | Checks if the Engine Room API is online |
| `get_settings` | `() → Config` | Reads the current `~/.aether/config.json` |
| `save_settings` | `(config: Config) → ()` | Writes updated config to `~/.aether/config.json` |
| `fetch_model_manifest` | `(url: string) → ModelManifest` | Fetches the curated model registry manifest |
| `download_model_from_manifest` | `(entry: ModelEntry) → ()` | Downloads and verifies a model from the registry |
...
| `--surface` | `rgba(22, 27, 34, 0.7)` | Header, sidebars, overlay backgrounds (glassmorphism) |
| `--card` | `rgba(28, 35, 51, 0.5)` | Pathway cards, Nexus cards (glassmorphism) |
| `--border` | `rgba(48, 54, 61, 0.6)` | All borders and dividers |
...
| `--neon-glow` | `0 0 15px rgba(88, 166, 255, 0.3)` | Active element glow (box-shadow) |
| `--transition-fluid` | `all 0.4s cubic-bezier(...)` | Standard interaction transition |

> **Note:** `--surface`, `--card`, and `--border` use `rgba()` with alpha transparency to support `backdrop-filter: blur(12px)` glassmorphism. The underlying RGB values match the GitHub Dark High-Contrast theme.
