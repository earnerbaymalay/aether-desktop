...
const SetupWizard: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            setScanPulse(true);
            try {
                // Real hardware interrogation
                const data = await AetherClient.getSystemStats();
                setStats(data);
                localStorage.setItem('aether_sys_stats', JSON.stringify(data));
            } catch (err) {
                console.error("Hardware interrogation failed", err);
    ...
    return (
        <div className="nexus-overlay">
            <div className="nexus-content glass border-glow-purple" style={{ maxWidth: '900px', width: '95%' }}>
            ...
            <style>{`
                ...
                .stat-box { display: flex; flex-direction: column; align-items: center; padding: 25px; background: rgba(255, 255, 255, 0.02); border: 1px solid var(--border); border-radius: 16px; transition: all 0.3s ease; }
                .stat-box:hover { background: rgba(255, 255, 255, 0.05); border-color: rgba(255, 255, 255, 0.2); transform: translateY(-4px); }
                .stat-box .label { font-size: 10px; color: var(--text-dim); margin-bottom: 8px; letter-spacing: 1.5px; }
                .stat-box .value { font-size: 26px; font-weight: 800; font-family: 'JetBrains Mono'; color: var(--text); }

                .calibration-view { display: flex; align-items: center; justify-content: space-around; padding: 60px 0; }
                .calibration-circle { width: 180px; height: 180px; border: 1px solid rgba(188, 140, 255, 0.3); border-radius: 50%; position: relative; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 30px rgba(188, 140, 255, 0.1); }
                .inner-pulse { width: 50px; height: 50px; background: var(--purple); border-radius: 50%; box-shadow: 0 0 40px var(--purple); animation: pulse 2s infinite; }

                .skill-selection-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 24px; }
                .skill-item { padding: 24px; border: 1px solid var(--border); border-radius: 16px; background: rgba(255, 255, 255, 0.02); text-align: center; position: relative; transition: all 0.3s ease; }
                .skill-item.active { border-color: var(--teal); background: rgba(0, 242, 255, 0.03); }
                .skill-item.pro { border-color: var(--purple); opacity: 0.6; }
                .skill-item:hover { transform: scale(1.02); background: rgba(255, 255, 255, 0.04); }
